...
3. On submit the page POSTs JSON to `/api/lead`, a Vercel serverless function
   on the same origin (so there is no CORS to configure).
4. The function validates, normalises the phone to `+91XXXXXXXXXX`, inserts one
...
4. **Environment Variables** → add `DATABASE_URL`, the same value as in
   `backend/.env`, for Production (and Preview, if you want previews to write).
5. Deploy.
...
```bash
npm install
cp .env.example .env.local    # paste the real DATABASE_URL
...
- **Connection footprint.** Each warm function instance holds at most one
  ...
  can't exhaust the database's connection limit and starve the main app. If
  this page ever gets heavy sustained traffic, move it behind the Neon pooler
  endpoint (`-pooler` in the host name).
